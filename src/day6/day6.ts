...
export const calculateSearchBlock = (
    ...
    if (endPos < line.length){
        searchString = searchString + line.substring(Math.max(startPos,0), endPos+1);
...
export const calculateBlock = (line: string, distance: number, repeatStart: boolean, repeatEnd: boolean): number =>{
    ...
    for (var index = 0; index< line.length; index++){
        ...
        // lowercase
        if (charCode>= 97){
            var searchString = calculateSearchBlock(line, index, distance, repeatStart, repeatEnd);
 ...
 */
export const countPairsinTents = (line: string, distance: number, repeat: number): number => {
    ...
    if (repeat ==2){
        return calculateBlock(line, distance, false, true) + calculateBlock(line, distance, true, false)
    }
