...
export const findChild =  (lines: string[]) : number =>{
    ...
    for (var lineIndex = 0; lineIndex< lines.length; lineIndex ++){
        ...
        var count =  lines[lineIndex].split("").filter( (it, index) =>  it == lines[comp1][index] || 
                it == lines[comp2][index]).length;
        if (count> maxCount){
            child = lineIndex
            maxCount = count;
        }
...
export const findScores =  (lines: string[]) : number[] =>{
    ...
    for (var lineIndex = 0; lineIndex< lines.length; lineIndex ++){
        ...
        var comparisons = lines.removeAtIndex(lineIndex).map (it => it.substring(2))
        var line = lines[lineIndex].substring(2).split("");

        var counts = comparisons.map( it => it.split("").filter( (char, index) => char == line[index] ).length)
...
const findParentForChild = (lines: string[], childIndex : number): string|undefined =>{
    ...
    for (var par1 = 0; par1< lines.length; par1 ++){
        ...
        for (var par2 = 0; par2< lines.length; par2 ++){
            ...
            var count = child
                    .filter ( (it, index) => it == parent1[index] || it == parent2[index] )
                    .length;
...
export const calculateScore = (child: string, parent: string): number =>{
    ...
    return parent.substringAfter(":")!.split("").filter( (item, index) => item == childData[index]).length
}
...
export const findParents3=  (lines: string[]) : number =>{
    ...
    for (var childIndex = 0; childIndex< lines.length; childIndex ++){
        ...
        if (parents){
            ...
            var found = families.filter (set => set.has(child) || set.has(parent1) || set.has(parent2) )
            if (found.length ==0){
                ...
                if (found.length>1){
                    for (var others =1 ; others< found.length; others++){
                        found[others].forEach(entry => found[0].add(entry))
